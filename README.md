...
`ROOT_URLCONF` and `WSGI_APPLICATION` have calculated defaults relative to the base module of your `DJANGO_SETTINGS_MODULE` if it is not a default `pydantic_settings` class. So, for example, if your `DJANGO_SETTINGS_MODULE` is set to `my_awesome_project.settings.PydanticSettingsSubclass`, then `ROOT_URLCONF` and `WSGI_APPLICATION` will be set to `my_awesome_project.urls` and `my_awesome_project.wsgi` respectively. This default behavior can be overridden by simply specifying `ROOT_URLCONF: str = 'the_actual_urlconf'` and `WSGI_APPLICATION: str = 'the_actual_wsgi_file.application'` in your `PydanticSettings` sub-class. If you *do* use a default `pydantic_settings` class but provide a `BASE_DIR`, then the default values of the `ROOT_URLCONF` and `WSGI_APPLICATION` settings will be calculated based on the directory name.
